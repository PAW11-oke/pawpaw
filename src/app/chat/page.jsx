...
export default function ChatPage() {
    ...
    useEffect(() => {
        socket = io(); // Initialize connection to Socket.IO server
        return () => {
    ...
    // Join selected group and listen for messages
    useEffect(() => {
        if (selectedGroupId) {
            setLoading(true);
            ...
            return () => {
                socket.emit('leaveGroup', selectedGroupId); // Leave group when group changes
                socket.off('newMessage'); // Clear listener when component unmounts
            };
        }
    }, [selectedGroupId]);
